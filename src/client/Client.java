...
System.out.println("받은 메세지 : "+msg);
		...
		else if(protocol.equals("gameover"))
		{
			...
			else //동점
			{
				highscore=scoreB;
				lowscore=scoreA;
				JOptionPane.showMessageDialog(null, "내 점수: "+highscore+"\n상대방 점수"+lowscore, "Draw", JOptionPane.INFORMATION_MESSAGE);
				player.setPlayGameNum(0);
				player.setStatus(null);
				gf.changePanel("menu");
				return;
			...
			{
				JOptionPane.showMessageDialog(null, "내 점수: "+highscore+"\n상대방 점수"+lowscore, "you win", JOptionPane.INFORMATION_MESSAGE);
			}
			else
			{
				JOptionPane.showMessageDialog(null, "내 점수: "+lowscore+"\n상대방 점수"+highscore, "you lose", JOptionPane.INFORMATION_MESSAGE);
			}
			player.setPlayGameNum(0);
			player.setStatus(null);
			gf.changePanel("menu");
