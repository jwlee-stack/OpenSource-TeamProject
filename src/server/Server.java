...
public class Server {
	...
	/**
	 * 해당 Server 클래스를 실행시킬 컴퓨터의 ip로 port번호를 사용해서 서버를 실행시킨다.
	 * 
	 * @param port - 포트 번호
	...
	/**
	 * 서버로부터 접속한 클라이언트의 소켓과 연결된 입출력 스트림 등 클라이언트의 정보가 들어 있는 클래스이다. Server클래스 내에서만
	 * 사용되는 클래스이며, 클라이언트와 통신하기 위해 만들어 졌다.
	 * 
	 * @author Ted
	 *
	 */
	private class ClientInfo {
		...
		/**
		 * 클라이언트로 메세지를 보내는 메소드이다. 메세지의 형태는 String이다. 메세지의 형태는 정해져 있으며, 규칙에 맞게 보내야만 한다.
		 * 
		 * @param msg
		 */
		...
		/**
		 * 클라이언트로부터 받은 메세지를 분해해서 의미를 파악하고 이에 대해 행하는 메소드이다. 행하는 방식은 메세지의 형태 즉, 프로토콜마다 다르며
			...
			} else if (protocol.equals("makeArray")) // 게임 배열 초기화
			{
				...
				for (int k = 0; k < getsu; k++) {
					for (int j = 0; j < getsu; j++) {
						this.myroom.broadcast("SetGame/" + Integer.toString(k) + "/" + Integer.toString(j) + "/"
								+ first_c.answer[k][j]);
	...
	/**
	 * 게임이 진행될 때, 두 상대방의 ClientInfo 객체를 저장시키는 클래스이다. 하나의 미니게임방이라 생각하면 된다. 해당 게임방에
	 * 존재하는 소켓들끼리만 데이터 통신을
	 * 
	 * @author Ted
	 *
